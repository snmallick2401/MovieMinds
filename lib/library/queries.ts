...
export const getLibraryDashboard = async (
  ...
  const getCachedDashboard = unstable_cache(
    ...
    [`library-dashboard-${userId}`],
    { tags: [`library-dashboard-${userId}`], revalidate: 3600 }
  );
  ...
  const getCachedStats = unstable_cache(
    async (id: string) => {
      const [completed, ratings, history, library] = await Promise.all([
        ...
        prisma.watchHistory.findMany({ where: { userId: id }, select: { watchedAt: true } }),
        prisma.userLibrary.findMany({ where: { userId: id }, select: { status: true } }),
      ]);
    ...
    [`user-stats-${userId}`],
    { tags: [`user-stats-${userId}`], revalidate: 3600 }
  );
