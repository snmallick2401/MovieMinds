...
export async function recalculateUserStats(userId: string) {
  const [aggregate, distribution, monthly, byType, byGenre] = await Promise.all([
    prisma.userRating.aggregate({ where: { userId }, _avg: { rating: true }, _count: { rating: true } }),
    getUserRatingDistribution(userId),
    prisma.userRating.findMany({ where: { userId }, select: { rating: true, createdAt: true } }),
    prisma.userRating.findMany({ where: { userId }, select: { rating: true, media: { select: { mediaType: true } } } }),
    prisma.userRating.findMany({ where: { userId }, select: { rating: true, media: { select: { genres: { include: { genre: true } } } } } }),
  ]);
